...
from GNN.loader.dataset.aqsol_molecules import AQSOL
from GNN.loader.dataset.coco_superpixels import COCOSuperpixels
from GNN.loader.dataset.malnet_tiny import MalNetTiny
from GNN.loader.dataset.voc_superpixels import VOCSuperpixels
from GNN.loader.split_generator import (prepare_splits,
                                             set_dataset_splits)
from GNN.loader.dataset.acl_arc import ACLCitationDataset
from GNN.transform.posenc_stats import compute_posenc_stats
from GNN.transform.task_preprocessing import task_specific_preprocessing
from GNN.transform.transforms import (pre_transform_in_memory,
...
def load_dataset_master(format, name, dataset_dir):
    ...
    if format.startswith('PyG-'):
        ...
        elif pyg_dataset_id == 'ACL-ARC':
            dataset = preformat_ACL_ARC(dataset_dir)
        else:
...
def preformat_OGB_Graph(dataset_dir, name):
    ...
    if name == 'ogbg-ppa':
        ...
        from GNN.loader.ogbg_code2_utils import idx2vocab, \
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary
        from GNN.loader.dataset.pcqm4mv2_contact import \
...
def preformat_Peptides(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary.
        from GNN.loader.dataset.peptides_functional import \
            PeptidesFunctionalDataset
        from GNN.loader.dataset.peptides_structural import \
...
def preformat_ACL_ARC(dataset_dir):
    """Load and preformat ACL-ARC datasets.
    """
    dataset = ACLCitationDataset(dataset_dir)
    return dataset
